...
const REDIRECT_DELAY_MS = 550;

/**
 * Primary CTA. Redirect target comes from `siteConfig.keySystemUrl`
 * (NEXT_PUBLIC_KEY_SYSTEM_URL) — change the destination there; nothing
 * in this component needs to know what that URL points to.
 *
 * Double-click protection is two layers deep on purpose: `hasFired` is
 * a ref checked synchronously, so a second click that lands before
 * React re-renders with `disabled` still gets blocked; the `disabled`
 * attribute is the visible, accessible layer once state catches up.
 */
export function GetKeyButton() {
  ...
  const handleClick = useCallback(() => {
    ...
    // Deliberate small delay: gives the loading state something to be
    // seen, and gives the analytics call above a moment to land before
    ...
    return (
      <div className="flex flex-col items-center gap-2">
        ...
        <p className="max-w-xs text-center text-xs text-ink-muted">
          NEXT_PUBLIC_KEY_SYSTEM_URL isn&apos;t set. Add it in your environment and redeploy.
        </p>
      </div>
    );
  }
